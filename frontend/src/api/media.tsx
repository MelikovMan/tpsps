...
export const getMediaList = async (params: MediaListParams = {}): Promise<MediaListResponse> => {
  ...
  // Вычисляем skip из page или берём явный skip
...
export const attachMediaToArticle = async ({ mediaId, articleId }: AttachMediaParams) => {
  await apiClient.post(`/media/${mediaId}/attach/article/${articleId}`);
};

export const attachMediaToCommit = async ({ mediaId, commitId }: { mediaId: string; commitId: string }) => {
  await apiClient.post(`/media/${mediaId}/attach/commit/${commitId}`);
};
